...
CANDIDATES = [
    'val_acc_top1', 'val_acc_top_1', 'val_top_1', 'val_acc_1',
    'val_accuracy', 'val_acc', 'val_acc_top_1%', 'val_1'
]
...
def main():
    ...
    for p in logs:
        ...
        summary.append({'model': model_name, 'final_val': final_val, 'val_column': val_col or ''})
    ...
    for model, (xs, ys, col) in series.items():
        ...
        ys_num = [float(y) if (y is not None and not math.isnan(y)) else math.nan for y in ys]
    ...
    with open(OUT_SUMMARY, 'w', newline='') as f:
        ...
        writer.writeheader()
        for s in summary:
            writer.writerow({'model': s['model'], 'val_column': s['val_column'], 'final_val': s['final_val']})
